...
image_width = 9
num_channels = 85
num_output_channels = 139
num_classes = num_output_channels * 9 * 9
...
def create_image_mb_source(map_file, is_training, total_number_of_samples):
    if is_training:
        ...
        count = 10000
        skip = 100000
    return PackedSfenDataSource(map_file, count=count, skip=skip, format_board=0, format_move=0,
...
def vgg16_train_and_eval(train_data, test_data, num_quantization_bits=32, minibatch_size=128, epoch_size=100000,
                         max_epochs=80,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--epoch_size', help='Epoch size', type=int, required=False, default='100000')
    ...
    parser.add_argument('-testing', '--testing',
                        help='Indicate if running for testing purposes (validation only done in a portion of the test dataset)',
                        action='store_true')
    ...
    train_data = r"D:\dev\shogi\db\shuffled_sfen1_1m.bin"
