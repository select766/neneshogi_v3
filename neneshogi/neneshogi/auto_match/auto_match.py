...
python -m neneshogi.auto_match.auto_match rule.yaml engine1.yaml engine2.yaml
"""
...
class AutoMatch:
    ...
    init_book_data: List[List[str]]  # 初期局面集 [[7g7f 3c3d...],[2g2f 8c8d...]...]
    ...
    def __init__(self, rule: Rule, engine_config_list: List[EngineConfig]):
        ...
        self._log_file = None
        self.watchdog = None
        self.init_book_data = None

    def _load_init_book(self):
        """
        初期局面集をメモリに読み込む
        :return:
        """
        if self.rule.init_book:
            self.init_book_data = []
            with open(self.rule.init_book) as f:
                for line in f:
                    ls = line.rstrip()
                    # startpos moves 7g7f 3c3d 6g6f ...
                    assert line.startswith("startpos moves ")
                    moves = ls.split(" ")[2:]
                    self.init_book_data.append(moves)
        else:
            self.init_book_data = [[]]

    def _log(self, msg: str):
        if self._log_file is not None:
            self._log_file.write(f"{datetime.now().strftime('%Y%m%d%H%M%S.%f')}:{msg}\n")
    ...
    def _set_init_book(self, board: shogi.Board):
        """
        初期局面集から1つ選んで盤面にセット
        :return:
        """
        init_book = random.choice(self.init_book_data)
        for move in init_book:
            board.push_usi(move)
    ...
    def _run_single_match(self, black_engine: int) -> MatchResult:
        ...
        winner = None
        gameover_reason = ""
        self._set_init_book(board)
        # 初期局面が後手の場合に対応。初形ならboard.move_number == 1なのでblack_engineが選ばれる。
        current_engine = black_engine if board.move_number % 2 == 1 else 1 - black_engine
        while True:
